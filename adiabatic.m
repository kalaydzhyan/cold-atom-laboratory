...
% Defining units
f0 = 1000;                                    % Units of frequency in Hz
omega0 = 2*pi*f0;
t0 = 1/omega0;                               % Units of time in s

% Initial trap parameters:
Omega_x = 1093/f0;                                 % Frequencies as measured in omega_0
Omega_y = 1064/f0;                                 
...
Type = 'Splitting';
%Type = 'Relaxation';
Totalt = 1;                                    % Total simulation time, in s
Deltat = .01;                                   % Time step of the simulation
...
Omega_x_i = 1093/f0;                                   % set this and next to 0 for a potential turned off
Omega_y_i = 1064/f0;
Omega_x_f = 0.489/f0;                                   % set this and next to 0 for a potential turned off
Omega_y_f = 0.476/f0;
Alpha_x = 0;      
Alpha_y = 0;
Physics2D = Physics2D_Var2d(Method, Delta, Beta);
...
Physics2D = TimePotential_Var2d(Method, Physics2D,...
           @(T,X,Y) quadratic_plus_cubic_potential2d(mass, ...
           (Omega_x_i+Omega_x_f)/2 + ((-Omega_x_i+Omega_x_f)/2)*tanh(5.8*(2*T / Stop_time)^(1/4)-1)/tanh(5.8), ...
           (Omega_y_i+Omega_y_f)/2 + ((-Omega_y_i+Omega_y_f)/2)*tanh(5.8*(2*T / Stop_time)^(1/4)-1)/tanh(5.8), ...
           Alpha_x, Alpha_y, X, Y));
